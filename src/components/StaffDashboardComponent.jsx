  ...
  return (
    <div className="min-h-screen bg-blue-50 px-4 py-10 relative">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-lg text-gray-600 mb-6">Interactive patient management for hospital staff</p>
            ...
            <tbody className="text-gray-800 text-sm">
              {mockPatients.map((patient) => (
                <tr key={patient.id} className="border-t">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${statusStyles[patient.currentStatus] || "bg-gray-100 text-gray-500"}`}
                    >
                  ...
                  <td className="px-6 py-4 space-x-3">
                    <Link to={`/admin/edit-patient?id=${patient.id}`} className="text-blue-600 hover:underline font-medium">
                      Edit
                    </Link>
                    <Link to={`/admin/update-status?id=${patient.id}`} className="text-green-600 hover:underline font-medium">
                      Update Status
                    </Link>
                  </td>
                </tr>
      ...
      <Link
        ...
        className="fixed bottom-10 right-10 bg-blue-500 text-white px-6 py-4 rounded-full shadow-lg flex items-center space-x-2 hover:bg-blue-900 transition"
      >
